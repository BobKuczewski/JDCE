...
import dce.*;
import shared.*;
public class CBARGRPH6 extends Ic {
  int state,last;
  public CBARGRPH6() {
    super();
    name=new String("Bar6");
    pin=new int[41];
    }
  public void reset() {
    super.reset();
  ...
  public int run() {
    int ret;
    ret=0;
    if (pin[1]==0 && pin[12]==1) state=1; else state=0;
    if (pin[2]==0 && pin[11]==1) state+=2;
    if (pin[3]==0 && pin[10]==1) state+=4;
    if (pin[4]==0 && pin[9]==1) state+=8;
    if (pin[5]==0 && pin[8]==1) state+=16;
    if (pin[6]==0 && pin[7]==1) state+=32;
