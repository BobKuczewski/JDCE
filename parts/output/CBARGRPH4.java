...
import dce.*;
import shared.*;
public class CBARGRPH4 extends Ic {
  int state,last;
  public CBARGRPH4() {
    super();
    name=new String("Bar4");
    pin=new int[41];
    }
  public void reset() {
    super.reset();
  ...
  public int run() {
    int ret;
    ret=0;
    if (pin[1]==0 && pin[8]==1) state=1; else state=0;
    if (pin[2]==0 && pin[7]==1) state+=2;
    if (pin[3]==0 && pin[6]==1) state+=4;
    if (pin[4]==0 && pin[5]==1) state+=8;
